...
public class ALNSSolution {
    ...
    public void insertCustomer(int routePosition, int insertCusPosition, Node insertCustomer) {
        ...
        for (int i = 1; i < insertRoute.getRoute().size(); i++) {
            ...
            if (time < insertRoute.getRoute().get(i).getReadyTime())
                time = insertRoute.getRoute().get(i).getReadyTime();
            else if (time > insertRoute.getRoute().get(i).getDueTime())
                timeWindowViolation += time - insertRoute.getRoute().get(i).getDueTime();
